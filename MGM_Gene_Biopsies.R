...
# Load the data
gene_data <- read.csv("~/School/WUR/SSB-80336 - Thesis/Provided Data/filt2ndHumanHgnc.csv", 
...
# When there is 'unknown', change it to NA
for (column in 1:ncol(data)) {
  # New R version:
  empty <- which(data[, column]=='unknown')
  data[empty, column] <- NA
}
...
for (i in 1:nrow(data)){
  ...
  # print(paste0(i,': ',round(percentage),'% ',rownames(data)[i]))
  if (percentage<=limit) {
    to_keep <- c(to_keep, i)
  }
}
...
#                         collection="Human genes from biopsies")
