...
colnames(data[, sapply(data, class) != "numeric"])
...
colnames(Annotation)<-c('NSTI type','Sex','Comorbidity',"Amputation", 
                        "Septicshock at baseline","IVIG","Clindamycin",'Death')
...
# Colour the genes and cytokines red and orange
for (cyto in cyto.names) {
  colours[which(colnames(data)==cyto)] <- '#E69F00'
}
for (gene in gene.names) {
  colours[which(colnames(data)==gene)] <- '#CC79A7'
}
...
qgraph_adj_mat <- qgraph(input=adj_matrix,
                         ...
                         palette="colorblind",
                         label.font='sans',
                         posCol=c("#009E73"), # colour of positive edges
                         negCol=c("#D55E00"), # colour of negative edges
...
createNetworkFromIgraph(igraph = igraph_adj,
                        ...
                        collection="Gene_cytokine_common")
